...
async function uploadStatic(creds, domain) {
  ...
  try {
    ...
    // Ensure root dir exists (first deploy). On re-deploy just overwrite files.
    const exists = await sftp.exists(remotePath);
    if (!exists) {
      await sftp.mkdir(remotePath, true);
    }
...
${routing}
    ...
    @images path *.webp *.webp *.webp *.gif *.webp *.avif *.svg *.ico
...
async function configureCaddy(creds, domain, type) {
  ...
  const checkResult = await sshExec(
    creds,
    `test -f ${configPath} && cat ${configPath} || echo __missing__`,
  );

  if (checkResult.trim() === siteConfig.trim()) {
    ok("Caddy config unchanged, skipping reload");
    return;
  }

  await sshExec(
    creds,
    `echo '${escaped}' | sudo tee ${configPath} > /dev/null && sudo systemctl restart caddy`,
  );
...
async function refreshRatings() {
  ...
  // Load existing ratings as fallback
  let existing = { yandex: { score: "4.9", count: 127 }, gis: { score: "4.8", count: 84 }, prodoc: { score: "5.0", count: 41 } };
  if (fs.existsSync(ratingsPath)) {
    try { existing = JSON.parse(fs.readFileSync(ratingsPath, "utf8")); } catch {}
  }

  const ratings = JSON.parse(JSON.stringify(existing));
  let fetched = 0;

  // 2GIS catalog API
  try {
    const r = await fetch(`https://catalog.api.2gis.com/3.0/items/byid?id=70000001062569003&fields=items.reviews,items.stat&key=demo`, {
    ...
    if (r.ok) {
      ...
      const reviews = item?.reviews || item?.stat;
      if (reviews?.general_rating != null) { ratings.gis.score = String(parseFloat(reviews.general_rating).toFixed(1)); fetched++; }
      if (reviews?.reviews_count != null)  { ratings.gis.count = reviews.reviews_count; fetched++; }
    }
  } catch {}

  // Yandex — try geosearch API
  try {
    const r = await fetch(
      `https://search-maps.yandex.ru/v1/?apikey=dda3ddba-c9ea-4ead-9010-f43321e53b0b&text=RA+RE+%D0%BA%D0%BB%D0%B8%D0%BD%D0%B8%D0%BA%D0%B0+%D0%90%D0%BD%D0%B0%D0%BF%D0%B0&lang=ru_RU&type=biz&results=5`,
    ...
    if (r.ok) {
      ...
      for (const f of features) {
        ...
        if (!meta) continue;
        const nameMatch = (meta.name || "").toLowerCase().includes("ra") || (meta.name || "").toLowerCase().includes("рэ") || (meta.name || "").toLowerCase().includes("re");
        if (!nameMatch) continue;
        if (meta.rating != null)       { ratings.yandex.score = String(parseFloat(meta.rating).toFixed(1)); fetched++; }
        if (meta.reviews_count != null){ ratings.yandex.count = meta.reviews_count; fetched++; }
        break;
      }
    }
  } catch {}

  ratings.updated = new Date().toISOString().slice(0, 10);
  fs.writeFileSync(ratingsPath, JSON.stringify(ratings, null, 2) + "\n");

  if (fetched > 0) ok(`Ratings updated (${fetched} values refreshed)`);
  else { log("Ratings API unavailable — keeping last known values"); }
}
...
async function main() {
  ...
  if (args.includes("--delete")) {
    ...
    await sshExec(
      creds,
      `sudo rm -f ${CADDY_SITES_DIR}/${domain} && sudo systemctl restart caddy`,
    );
  ...
  if (type === "static") {
    // Refresh platform ratings before uploading
    log("Fetching platform ratings...");
